...
//设置异步任务的工作进程数量
$serv->set(array('worker_num' => 4, 'task_worker_num' => 2));
...
$serv->on('receive', function($serv, $fd, $from_id, $data) {
    ...
    $mem->set('fdd', $fd);
    $mem->set('worker_id', $serv->worker_id);
    //投递异步任务
    $data = unserialize($data);

    //$task_id = $serv->task($data, 5);
...
$serv->on('task', function ($serv, $task_id, $from_id, $data) {
    ...
    $mem->addServer('127.0.0.1','11211');
    $fdd = $mem->get('fdd');
    $serv->send($fdd, $serv->worker_id - $serv->setting['worker_num']);
    $data['worder_id'] = $mem->get('worker_id');
